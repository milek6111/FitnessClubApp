...
import { getClubUsersFn, getMembershipInfo, getUsersFn } from "../api/endpoints";
...
export const ClubUsers = () => {
    ...
    const fetchUsers = () => {
        fetch(getClubUsersFn.path + "?id="+id).then(
            res => res.json()
        )
        .then(
            val => setUsers(val)
    ...
    const Membership = (props: {id: number}) =>{
        const [membership, setMembership] = useState<{id_klub: number, nazwa: string, data_zakupu: string, data_waznosci: string, oplata: number}[]>([])

        useEffect(() =>{
            fetch(getMembershipInfo.path + "?id=" + props.id)
            .then(res => res.json())
            .then(res =>{
                setMembership(res.sort((a:{id_klub: number, nazwa: string, data_zakupu: string, data_waznosci: string, oplata: number},
                                        b: {id_klub: number, nazwa: string, data_zakupu: string, data_waznosci: string, oplata: number}) => (
        ...
        const content = membership.filter( mem => mem.id_klub.toString() === id).map(obj => (
                <tr>
                    <td>{obj.nazwa}</td>
    ...
    return (
        <div>
            ...
            <Link to={path.substring(0,7) + "/manage/" + id}>
